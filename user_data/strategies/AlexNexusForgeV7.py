...
def calculate_confluence_score(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # Volume Confluence
    dataframe["volume_spike"] = (dataframe["volume"] > dataframe["avg_volume"] * 1.5).astype(int)
    ...
    dataframe["rsi_neutral"] = ((dataframe["rsi"] >= 40) & (dataframe["rsi"] <= 60)).astype(int)
...
def calculate_smart_volume(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # Volume trend
    dataframe["volume_trend"] = (dataframe["volume_sma20"] > dataframe["volume_sma50"]).astype(int)
...
def calculate_advanced_momentum(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe["momentum_acceleration"] = dataframe["momentum_3"] - dataframe["momentum_3"].shift(3)
...
def calculate_market_structure(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe["bullish_structure"] = (
        dataframe["higher_high"].rolling(5).sum() + dataframe["higher_low"].rolling(5).sum()
    )

    dataframe["bearish_structure"] = (
        dataframe["lower_high"].rolling(5).sum() + dataframe["lower_low"].rolling(5).sum()
    )

    dataframe["structure_score"] = dataframe["bullish_structure"] - dataframe["bearish_structure"]
    ...
    dataframe["structure_break_up"] = (dataframe["close"] > swing_highs.ffill()).astype(int)

    dataframe["structure_break_down"] = (dataframe["close"] < swing_lows.ffill()).astype(int)
...
def calculate_advanced_entry_signals(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # RSI position signal
    dataframe["rsi_signal"] = ((dataframe["rsi"] > 30) & (dataframe["rsi"] < 70)).astype(int)
...
class AlexNexusForgeV7(IStrategy):
        ...
        **kwargs,
        ...
        if not dataframe.empty:
            ...
            if current_volatility > 0.05:  # 5% ATR/Price ratio
                ...
                logger.info(
                    f"{pair} Stake reduced by {volatility_reduction:.1%} due to high volatility ({current_volatility:.2%})"
                )
        ...
        if self.position_adjustment_enable:
            ...
            if num_safety_orders > 0 and volume_scale > 0:
                ...
                logger.warning(
                    f"{pair}: Could not calculate max_dca_multiplier due to "
        ...
        if calculated_max_dca_multiplier > 0:
            ...
            logger.info(
                f"{pair} Initial stake calculated: {final_stake:.8f} (Proposed: {proposed_stake:.8f}, "
            ...
            if min_stake is not None and final_stake < min_stake:
                logger.info(
                    f"{pair} Initial stake {final_stake:.8f} was below min_stake {min_stake:.8f}. "
    ...
    def custom_entry_price(
        ...
        **kwargs,
        ...
        if dataframe.empty:
            logger.warning(
                f"{pair} Empty DataFrame in custom_entry_price. Returning proposed_rate."
            )
        ...
        if (
            ...
            and abs(entry_price - self.last_entry_price) < 0.000005
        ):
            increment_factor = (
                self.increment_for_unique_price.value
                if side == "long"
                else (1.0 / self.increment_for_unique_price.value)
            )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        logger.info(
            f"{pair} Custom SL: {final_stoploss:.3f} | ATR: {atr:.6f} | Profit: {current_profit:.3f} | Trailing: {trailing_offset:.3f}"
        )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        if trade.stake_amount >= max_total_stake:
            logger.info(
                f"{trade.pair} 🛑 MAX STAKE REACHED: {trade.stake_amount:.2f}/{max_total_stake} USDT"
            )
            return None
        ...
        trigger = getattr(
            self, "initial_safety_order_trigger", DecimalParameter(-0.02, -0.01, default=-0.018)
        ).value
        trigger = trigger * dca_multiplier

        # CORRECTED DCA LOGIC: DCA when we're LOSING money
        if (
            (current_profit > trigger / 2.0 and count_of_entries == 1)
            or (current_profit > trigger and count_of_entries == 2)
            or (current_profit > trigger * 1.5 and count_of_entries == 3)
        ):
            logger.info(
                f"{trade.pair} DCA not triggered. Profit {current_profit:.2%} > threshold {trigger:.2%}"
            )
            return None
        ...
        try:
            ...
            current_multiplier = (
                dca_multipliers[count_of_entries - 1]
    ...
    def leverage(
        ...
        **kwargs,
        ...
        current_sma = (
            sma_array[-1] if sma_array.size > 0 and not np.isnan(sma_array[-1]) else current_rate
        )
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe["minima_sort_threshold"] = (
            dataframe["close"].rolling(window=rolling_window_threshold, min_periods=1).min()
        )
        dataframe["maxima_sort_threshold"] = (
            dataframe["close"].rolling(window=rolling_window_threshold, min_periods=1).max()
        )
        ...
        dataframe["slope_5"] = (
            dataframe["close"].rolling(5).apply(lambda x: calc_slope(x, 5), raw=False)
        )
        dataframe["slope_10"] = (
            dataframe["close"].rolling(10).apply(lambda x: calc_slope(x, 10), raw=False)
        )
        dataframe["slope_20"] = (
            dataframe["close"].rolling(20).apply(lambda x: calc_slope(x, 20), raw=False)
        )
        ...
        dataframe["volume_momentum"] = (
            dataframe["volume"].rolling(3).mean() / dataframe["volume"].rolling(20).mean()
        )

        dataframe["rsi_divergence_bull"] = (dataframe["close"] < dataframe["close"].shift(5)) & (
            dataframe["rsi"] > dataframe["rsi"].shift(5)
        )
        dataframe["rsi_divergence_bear"] = (dataframe["close"] > dataframe["close"].shift(5)) & (
            dataframe["rsi"] < dataframe["rsi"].shift(5)
        )
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        short_quality_filters = (
            (
                dataframe["ultimate_score"] < (1 - self.ultimate_score_threshold.value)
            )  # Low score for shorts
        ...
        medium_quality_short = (
            ...
            & ~(
                ...
                | (
                    dataframe["ultimate_score"] > self.ultimate_score_threshold.value
                )  # Exclude high-quality long setups
            )
        )
        ...
        dataframe.loc[medium_quality_entry & ~high_quality_entry, "enter_tag"] = (
            "medium_quality_long"
        )
        ...
        dataframe.loc[backup_entry & ~(high_quality_entry | medium_quality_entry), "enter_tag"] = (
            "backup_long"
        )
        ...
        dataframe.loc[medium_quality_short & ~high_quality_short, "enter_short"] = 1
        dataframe.loc[medium_quality_short & ~high_quality_short, "entry_type"] = (
            7  # Medium quality short
        )
        dataframe.loc[medium_quality_short & ~high_quality_short, "enter_tag"] = (
            "medium_quality_short"
        )
        ...
        if metadata["pair"] in ["BTC/USDT:USDT", "ETH/USDT:USDT"]:
            recent_entries = (
                dataframe["enter_long"].tail(5).sum() + dataframe["enter_short"].tail(5).sum()
            )
            if recent_entries > 0:
                ...
                logger.info(
                    f"{metadata['pair']} ENTRY SIGNAL - Type: {entry_types.get(entry_type, 'Unknown')}"
                )
    ...
    def _populate_custom_exits_advanced(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        df.loc[
            any_long_exit & long_exit_structure_breakdown & (df["exit_tag"] == ""), "exit_tag"
        ] = "MML_Structure_Breakdown_Confirmed"
        df.loc[any_long_exit & long_exit_resistance_profit & (df["exit_tag"] == ""), "exit_tag"] = (
            "MML_Resistance_Profit"
        )
        df.loc[
            any_long_exit & long_exit_extreme_overbought & (df["exit_tag"] == ""), "exit_tag"
        ] = "MML_Extreme_Overbought"
        df.loc[any_long_exit & long_exit_volume_exhaustion & (df["exit_tag"] == ""), "exit_tag"] = (
            "MML_Volume_Exhaustion_Long"
        )
        df.loc[
            any_long_exit & long_exit_momentum_divergence & (df["exit_tag"] == ""), "exit_tag"
        ...
        if self.can_short:
            ...
            df.loc[
                any_short_exit & short_exit_structure_breakout & (df["exit_tag"] == ""), "exit_tag"
            ] = "MML_Structure_Breakout_Confirmed"
            df.loc[
                any_short_exit & short_exit_support_profit & (df["exit_tag"] == ""), "exit_tag"
            ] = "MML_Support_Profit"
            df.loc[
                any_short_exit & short_exit_extreme_oversold & (df["exit_tag"] == ""), "exit_tag"
            ] = "MML_Extreme_Oversold"
            df.loc[
                any_short_exit & short_exit_volume_exhaustion & (df["exit_tag"] == ""), "exit_tag"
            ] = "MML_Volume_Exhaustion_Short"
            df.loc[
                any_short_exit & short_exit_momentum_divergence & (df["exit_tag"] == ""), "exit_tag"
            ] = "MML_Momentum_Divergence_Short"
            df.loc[any_short_exit & short_exit_range & (df["exit_tag"] == ""), "exit_tag"] = (
    ...
    def _populate_simple_exits(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if metadata["pair"] in ["BTC/USDT:USDT", "ETH/USDT:USDT"]:
            recent_exits = (
                dataframe["exit_long"].tail(5).sum() + dataframe["exit_short"].tail(5).sum()
            )
            if recent_exits > 0:
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        if trade_duration > 48:
            logger.info(
                f"{pair} Forcing exit due to max holding time (48h). Profit: {current_profit_ratio:.2%}"
            )
            return True
